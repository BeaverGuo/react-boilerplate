...
var VALID_TAG_REGEX = /^[a-zA-Z][a-zA-Z:_\.\-\d]*$/; // Simplified subset [a-zA-Z]匹配字符集,一个字母加字母或者' : _ . - 数字 ' 
var validatedTagCache = {};
var hasOwnProperty = {}.hasOwnProperty;//Object.prototype.hasOwnProperty for convience.such as [].map.call,yeah!
...
function ReactDOMComponent(element) {
  var tag = element.type;//'div'
  validateDangerousTag(tag);
  this._currentElement = element;//这里存着组件
  ...
  this._wrapperState = null;//这个是parent的state?
...
ReactDOMComponent.Mixin = {
   ...
   */
  mountComponent: function (transaction, nativeParent, nativeContainerInfo, context) {//context是history,store这些,nativeParent是sideBox
    ...
    var mountImage;
    if (transaction.useCreateElement) {//transaction是什么?
      ...
      if (namespaceURI === DOMNamespaces.html) {
        if (this._tag === 'script') {
          ...
          var div = ownerDocument.createElement('div');//why ownerDocument?防止出现多个window的情况?
      ...
      this._flags |= Flags.hasCachedChildNodes;//有无子项
  ...
  _createInitialChildren: function (transaction, props, context, lazyTree) {
    ...
    if (innerHTML != null) {
      ...
      var contentToUse = CONTENT_TYPES[typeof props.children] ? props.children : null;//找出string and number
      var childrenToUse = contentToUse != null ? null : props.children;
      if (contentToUse != null) {
        // TODO: Validate that text is allowed as a child of this node
        if (process.env.NODE_ENV !== 'production') {
   ...
   */
  _updateDOMProperties: function (lastProps, nextProps, transaction) {//初次mount lastProps为null ,2个div,ReactReconcileTransaction
    var propKey;
    var styleName;
    var styleUpdates;
    for (propKey in lastProps) {//这是跳过新的有,旧的没有,旧的为null的
      if (nextProps.hasOwnProperty(propKey) || !lastProps.hasOwnProperty(propKey) || lastProps[propKey] == null) {
        continue;
      }
      if (propKey === STYLE) {//样式属性处理
        var lastStyle = this._previousStyleCopy;
        for (styleName in lastStyle) {
          if (lastStyle.hasOwnProperty(styleName)) {
            styleUpdates = styleUpdates || {};
            styleUpdates[styleName] = '';//已经有的样式名置为空串
          }
        }
        this._previousStyleCopy = null;
      } else if (registrationNameModules.hasOwnProperty(propKey)) {//registrationNameModules貌似是react事件对象数组
        if (lastProps[propKey]) {//移除上一次的事件?垃圾回收?
